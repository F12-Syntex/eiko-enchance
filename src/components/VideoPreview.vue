...
const videoStyle = computed(() => ({
    ...
    transform: isSoBActive.value ? 'translateX(0)' : 'none'
}));
...
const startAudioAnalysis = () => {
    ...
    const smoothingFactor = 0.95; // Adjust this for more/less smoothing
    const beatThreshold = 1.1; // Adjust this to change beat sensitivity
    ...
    const analyzeAudio = () => {
        ...
        rollingAverage = smoothingFactor * rollingAverage + (1 - smoothingFactor) * currentAverage;
        console.log(rollingAverage, currentAverage);
        

        // Check if current average is significantly higher than rolling average
        if (currentAverage > beatThreshold * rollingAverage) {
            isSoBActive.value = true;
            applySOBEffect();
            
        } else {
            isSoBActive.value = false;
            removeSOBEffect();
        }
    };
...
const applySOBEffect = () => {
    if (videoElement.value) {
        videoElement.value.style.transform = `translateX(${Math.random() * 10 - 5}px) translateY(${Math.random() * 10 - 5}px)`;
    }
};

const removeSOBEffect = () => {
    if (videoElement.value) {
        videoElement.value.style.transform = 'translateX(0) translateY(0)';
    }
};
